...
from environment import MRubisEnvironment
import numpy as np
...
class MRubisController():
    ...
    def _update_number_of_issues_in_run(self):
        ...
        self.number_of_issues_in_run = self.environment.get_number_of_issues_in_run()
    ...
    def _send_rule_to_execute(self, issue, rule):
        ...
        self.environment.send_rule_to_execute(
            shop_name, issue_name, component_name, rule)
            ...
            for issue_name, component in fixed_components:
                ...
                if fixes_can_fail:
                    failing_component_types = [
                        values[1] for component_info in self.components_fixed_in_this_run.values() for values in component_info]
                    type_of_component_to_fix = fixed_components[0][1]
                    fail_probability = self.component_dependency_model.fix_fail_probability(
                        type_of_component_to_fix, failing_component_types)
    ...
    def _cumulative_utility_based_on_order(self, sorted_df: pd.DataFrame):
        ...
        cumulative_utility_per_component = [
            util * (n_rows - i) for i, util in enumerate(sorted_df['predicted_optimal_utility'])]
    ...
    def run(self, external_start=False, max_runs=300, rule_picking_strategy='lowest', issue_ranking_strategy='utility', fixes_can_fail=False):
        ...
        while self.run_counter < max_runs:
            ...
            while self.number_of_issues_handled_in_this_run < self.number_of_issues_in_run:
                ...
                current_issue = self.environment.get_current_issue()
            ...
            # Side effect: [shop][component]['predicted_optimal_utility']
            self._predict_optimal_utility_of_fixed_components(fixes_can_fail)
            ...
            fixed_issues = self._cumulative_utility_based_on_order(
                self.current_fixes)
            self.fix_history.append(fixed_issues)

            logger.info(f'Fixing in this order: {component_fixing_order}')
            self.environment.send_order_in_which_to_apply_fixes(
                component_fixing_order)
